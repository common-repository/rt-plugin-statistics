...
/*
	...
	Text Domain: rt-plugin-statistics
...
if( !class_exists('RT_Plugin_Stats') && is_multisite() ){
	...
	class RT_Plugin_Stats {
		...
		function load_textdomain(){
			load_plugin_textdomain( 'rt-plugin-statistics', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
		}
		 ...
		 */
		function admin_menu() {
			add_submenu_page( 'settings.php', __( 'Plugin Statistics', 'rt-plugin-statistics' ), __( 'Plugin Statistics', 'rt-plugin-statistics' ), 'manage_options', 'rt_plugin_stats', array( $this, 'settings_page' ) );
		}
		...
		function settings_page() {
			...
			echo '<div class="wrap">';
			echo '<h2>' . __( 'Plugin Statistics', 'rt-plugin-statistics' ) . '</h2>';
			echo '<h3>' . __( 'Network activated plugins', 'rt-plugin-statistics' ) . '</h3>';
			echo '<p>';
			...
			echo '<h3>' . __( 'Activated plugins', 'rt-plugin-statistics' ) . '</h3>';
			echo '<p>';
			...
			echo '</p>';
			echo '<p><em>';
			printf( __('Page render time: %1$s seconds, sites queried: %2$s', 'rt-plugin-statistics' ), round( microtime( true ) - $starttime, 3 ), count( $sites ) );
			echo '</em></p>';
			echo '</div>';
		...
		function render_network_activated_table( $results ){
			...
			$html .= '<thead>';
			$html .= '<tr>';
			$html .= '<th class="manage-column column-columnname">' . __( 'Plugin name', 'rt-plugin-statistics' ) . '</th>';
			$html .= '</tr>';
			$html .= '</thead>';
			$html .= '<tbody>';
		...
		function render_table( $results ){
			...
			$html .= '<tr>';
			$html .= '<th class="manage-column column-columnname">' . __( 'Plugin name', 'rt-plugin-statistics' ) . '</th>';
			$html .= '<th class="manage-column column-columnname num">' . __( 'Activation count', 'rt-plugin-statistics' ) . '</th>';
			$html .= '<th class="manage-column column-columnname">' . __( 'Sites', 'rt-plugin-statistics' ) . '</th>';
			$html .= '</tr>';
			$html .= '</thead>';
			$html .= '<tbody>';
			...
			foreach( $results as $name=>$inst ){
				...
				$html .= '</div>';
				$html .= '<a class="rt_plugin_stats_toggle_details" href="#">' . __( 'show', 'rt-plugin-statistics' ) . '</a>';
				$html .= '</td>';
		...
		function render_inline_js(){
			...
			$html .= '$(this).closest("td").find(".rt_plugin_stats_toggle_details").html("' . __( 'show', 'rt-plugin-statistics' ) . '")';
			$html .= '} else {';
			$html .= '$(this).closest("td").find(".rt_plugin_stats_toggle_details").html("' . __( 'hide', 'rt-plugin-statistics' ) . '")';
			$html .= '}';
			$html .= '});';
			$html .= '});';
			$html .= '});';
